...
- System/version: formal brokered review controller schema 2 and spend-ledger terminal cleanup; standard schema 1 remains outside cleanup scope.
...
| Apply formal cleanup to a standard review | Standard controller reaches a graceful terminal | Compatibility and accounting drift | Controller gates retirement on formal schema 2; standard terminal and hold regressions preserve schema 1 | Controller, approval, launch and conformance tests | Standard unused holds remain conservative by contract |
...
| Hide a transfer by losing its record | Crash after ledger transfer | Incomplete attribution | Atomic retirement reports whether it changed the source; only a formal controller can write the schema-2 terminal cleanup marker, and inspection requires the exact formal start/envelope scope while a pre-retired source without a transfer record remains incomplete | Inspection tests | Target identity cannot be reconstructed without retained record |
