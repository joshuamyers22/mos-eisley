...
- Full quality-gate command and result: unrestricted `make check` passed Ruff,
  formatting, Pyright, 2,476 source tests with four skips, 89% coverage, export and
  distribution builds, and 1,852 installed-wheel tests. The sandboxed run reached
  all source tests but reported 31 localhost fixture errors because socket binding
  was denied. The first unrestricted run had one transient missing runtime-start
  fixture; its exact test and the complete clean rerun passed.
...
- Durable facts promoted to tests, ADRs, docs, or `PROJECT_MEMORY.md`: recursive
  ...
  documentation, qualification history, and project memory.
