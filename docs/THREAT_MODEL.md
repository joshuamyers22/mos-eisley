...
| A passing skill report silently changes runtime behavior | Reports deny promotion; archive binding, release control, and quarantine staging deny deployment; a separate independent signature can authorize only one exact staged package and inert target | No shipped consumer installs, changes a default, activates runtime use, or monitors drift |
...
| One installation authorization is replayed or survives revocation | The signature binds one claim-store identity, exact target, staged manifest, control entry, and short window; the private ledger consumes it durably before side effects while a latest-control read guard remains held through caller commit | The owner can delete, roll back, or clone the local claim/anchor stores; there is not yet an installer, external monotonic witness, or completed-install record |
