...
| A passing skill report silently changes runtime behavior | Reports deny promotion; archive binding and release control gate exact bytes; separate independent one-use authorities gate inert installation and one state-bound default-pointer change | No runtime lookup, activation, health check, or drift monitor exists |
...
| A selected default is treated as active | Pointer/results explicitly deny runtime lookup, activation, and every other configuration change, and no runtime code reads the default store | Runtime consumption, health evidence, and drift-triggered rollback remain future gates |
