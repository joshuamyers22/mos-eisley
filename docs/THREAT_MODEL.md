...
| A passing skill report silently changes runtime behavior | Reports deny promotion; independently signed promotion receipts still literally deny activation and configuration mutation | Archives are retained but not bound to receipts; rollback, installation, revocation, and drift gates are not implemented |
