...
| A statistically eligible route silently becomes active | Scoring and holdout contracts deny promotion; the separately authenticated promotion receipt still has literal activation denial and no runtime/configuration capability | A future time-bounded activation boundary requires drift, availability, rollback, and separate authorization review |
