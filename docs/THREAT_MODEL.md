...
| A passing skill report silently changes runtime behavior | Reports deny promotion; archive binding and release control gate exact bytes; independent one-use authority can install only into a private inert content store | No default pointer, runtime lookup, activation, or drift monitor exists |
