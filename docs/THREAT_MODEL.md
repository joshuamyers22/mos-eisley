...
| A revoked policy reuses an older control message | Signed sequence floor, evidence-age bound, emergency stop, explicit revocations, and earliest-expiry receipt limit replay | Without an external monotonic latest-sequence anchor, an older still-valid state remains replayable until expiry |
