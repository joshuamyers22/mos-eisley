...
| A statistically eligible route silently becomes active | Both scoring contracts have literal `promotion_ready=false`; scorer has no registry, configuration, provider or publishing capability | A future policy derivation and activation boundary requires separate authorization and review |
