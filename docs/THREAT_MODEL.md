...
| Observation compilation discards authenticated grading lineage | Separate compiler reconstructs the full private execution chain, reverifies dual grading, and binds observations to both policies and the complete resolution digest | Source artifact retention is operator-controlled; the new schema remains intentionally unaccepted by scoring |
