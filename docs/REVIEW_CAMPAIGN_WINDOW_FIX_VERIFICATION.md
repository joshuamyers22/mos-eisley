...
- Revision lineage: `bac7dd24dc5a8a5e20b4b79a229dffc73882c88c` was the original implementation base; `0010970a8960161266841dc59a64c0ef58c48221` is the original implementation and accepted live-campaign revision. The scoped-window correction started from clean descendant `c29e46bee58e8c109a5dc8b49c15f8534cee07fc` and was committed as `6d079cadf2a57da54acf445b6c364ad7aad50bd4`. The deferred-judge scope and harness correction is exact commit `9300de46bc67bcbfeccdee061599130cceae8d17`.
...
- Maximum iterations: eight evidence-changing passes, including bounded corrections and immutable-image verification prompted by offline or retained campaign evidence.
...
| 6 | Reopened by fresh `6d079ca` campaign slot 1 | Three correlated critics and the judge rejected because the envelope made judge scope inheritance implicit rather than an independently validated commitment | W-004 blocker | Commit the planned judge scope in the allowance and reject mismatch at envelope, campaign, probe and retained-transfer boundaries | Four new tamper regressions plus focused/full offline gates |
| 7 | Installed-wheel stress hardening | The complete source suite passed, but the first 1,777-test installed-wheel run exposed one intermittent observer-handoff fixture failure consistent with a synthetic 20-second phase certificate being exhausted before the broker established lifecycle evidence | W-005 test reliability | Preserve the short default for expiry tests; have only formal-campaign fixtures request the authority policy's bounded 60-second lifetime, still clipped by the exact phase scope, and assert both bounds explicitly | 26 campaign-runner/observer tests pass; 20 admission/expiry tests pass; all 1,777 installed-wheel tests pass in 989 seconds |
| 8 | Corrected production-image rebuild | Clean exact `9300de4`, pinned build inputs, complete container gate and separate immutable-image inspection | No finding | Pin the new image for the next wholly fresh campaign; do not treat image verification as campaign or launch evidence | `make container` passes; image `sha256:c7f634464887b534de8ee6b803c52c3fe65afb665b0314db2e9385c77e16b990`; installed corrected source hashes match the checkout |
...
| W-004 | Fresh `6d079ca` campaign result `aa2bcdc27a785934b6dbfeea70572f8f4cf1904be058326de6fad00c312a0d4e` | Judge scope inheritance existed by construction but was not committed in the spend-only allowance or independently rechecked during retained transfer verification | blocker | Accepted as defense-in-depth hardening: bind the planned scope before reservation and validate it at every campaign/judge boundary | Envelope mismatch, probe admission, campaign substitution and retained-transfer tamper tests | Joshua Myers |
| W-005 | Installed-wheel `ObserverHandoffTests` setup after 1,776 preceding tests | A scheduler stall can consume a synthetic 20-second certificate, producing a fail-closed result without the lifecycle evidence required for observer handoff | low | Accepted as harness hardening: formal campaign fixtures request the already-enforced 60-second authority maximum while exact scope expiry still clips it; ordinary expiry tests retain 20 seconds | Explicit authority/scope lifetime assertions plus campaign-runner, observer-handoff, admission and installed-wheel suites | Joshua Myers |

## Exit

- Stop reason: W-004 product hardening and W-005 harness hardening are implemented and verified offline after cancelling the fresh campaign before slot 2.
- Rubric result and blocking findings: W-001 through W-003 remain resolved; W-004 regressions and the complete source suite pass. W-005's bounded synthetic-certificate timing weakness is hardened without changing product dispatch authority. No blocking finding remains in the offline slice.
- Full quality-gate command and result: Ruff, format and Pyright pass. The authoritative loopback-enabled source run passed all 2,419 tests with 4 skips and 89% coverage. Export verification and sdist/wheel build pass. After W-005 hardening, all 1,777 installed-wheel tests pass in 989.228 seconds.
- Production-like replay/fault/rollback evidence: the exact `0010970a` image completed a three-slot campaign with campaign status `accepted`; content verdicts were `reject`, `revise`, `reject` and supplied W-002/W-003. The later `6d079ca` campaign is terminal and incomplete after slot 1 supplied W-004. Exact commit `9300de46bc67bcbfeccdee061599130cceae8d17` was rebuilt as immutable Linux/arm64 image `sha256:c7f634464887b534de8ee6b803c52c3fe65afb665b0314db2e9385c77e16b990`; the complete offline container gate passed. Separate network-disabled, read-only inspection confirmed UID/GID `10001:10001`, Python 3.12.14, Mos Eisley 0.1.0, OpenAI SDK 3.11.0, 600/1,800-second constants, exact matching hashes for `review_broker.py`, `review_campaign.py` and `review_conformance_probe.py`, no Git/compiler/pytest/tests/source tree, no declared volumes or ports, and no remaining image-derived container. No provider call was made.
- Remaining uncertainty, owners, and dates: current-artifact qualification and the separate launch decision remain open; a wholly fresh campaign must bind exact commit `9300de46bc67bcbfeccdee061599130cceae8d17` and image `sha256:c7f634464887b534de8ee6b803c52c3fe65afb665b0314db2e9385c77e16b990`; Joshua Myers.
