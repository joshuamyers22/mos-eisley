...
| A nearby prompt, model, or effort is substituted | Installed bytes reconstruct the exact `PromptAsset`; complete route equality, registry identity, no-substitution resolution, and full routing-source recomputation are required | Retained source and signer trust remain external |
...
Milestone 34 implements the non-sending full-lineage and guarded admission portion. An
independently authorized capability, provider send, and conservative settlement remain
open.
