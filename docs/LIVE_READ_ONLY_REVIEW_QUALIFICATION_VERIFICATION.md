...
| Q-008 | The generic strict-schema normalizer only hardens object nodes whose `properties` member is already a dictionary | An arbitrary object-shaped schema can escape recursive normalization or fail later at the provider boundary | Blocking in the latest review; the exact live role schemas were fully strict | Reject or normalize unsupported object shapes explicitly and add nested missing/malformed-properties regressions without weakening local decoding | Focused schema tests plus complete package/container gates | Joshua Myers |
| Q-009 | The three-critic fault regression stops after the judge and does not construct and authenticate a signed post-result observation | The complete claimed lifecycle lacks deterministic one-invalid-of-three observation coverage | High; live path succeeded but deterministic coverage is incomplete | Extend the fault regression through semantic exchange collection and signed observation authentication, including tamper rejection | Focused full-lifecycle regression plus complete package/container gates | Joshua Myers |
| Q-010 | The standalone retest retained the signed observation but not the authority policy, observation policy, or signed phase authorizations required for later `authenticate_review_probe` replay | The in-process construction/signing success cannot become independently replayable historical authentication evidence | High operational evidence gap; no effect on the immutable result or accounting | Preserve the limitation; require any future harness to atomically retain the complete authentication input set before discarding its ephemeral key | Offline reconstruction/authentication from only retained artifacts | Joshua Myers |
...
- Rubric result and blocking findings: Q-003 remains accepted by the accountable owner;
  ...
  Q-009. No new run or qualification authority follows.
